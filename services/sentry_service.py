...
from config.sentry import (
    capture_custom_exception as _capture_custom_exception,
    log_custom_message as _log_custom_message,
)
...
def capture_custom_exception(e: Exception, context: dict = None) -> None:
    ...
    try:
        ...
        error_logger.error(
            f"Failed to capture exception to Sentry: {str(inner_e)}", exc_info=True
        )
...
def log_custom_message(message: str, level: str = "info", context: dict = None) -> None:
    ...
    """
    app_logger.info(
        f"Logging custom message to Sentry [Level: {level}]: {message[:100]}..."
    )
    try:
