...
class BaseService:
    ...
    def _to_schema(self, document: TModel) -> TSchema:
        try:
            ...
            error_logger.error(
                f"Schema validation failed for Model {self.model.__name__}: {str(e)}",
                exc_info=True,
            )
            raise ValidationError(
                f"Data corruption detected in DB for {self.model.__name__}",
                details={"error": str(e)},
            )

    def get_by_id(
        self, doc_id: str, organization_id: str = None, fields: List[str] = None
    ) -> TSchema:
        app_logger.debug(
            f"Entering get_by_id for {self.model.__name__}: {doc_id} (org: {organization_id})"
        )
        try:
            filters = {"id": doc_id}
            if organization_id:
            ...
            app_logger.debug(
                f"Exiting get_by_id for {self.model.__name__}: {doc_id} successfully"
            )
            return result
        except (DoesNotExist, ValueError):
            app_logger.warning(
                f"{self.model.__name__} with id {doc_id} (org: {organization_id}) not found."
            )
            raise NotFoundError(f"{self.model.__name__} not found")
        except Exception as e:
            error_logger.error(
                f"Error in get_by_id for {self.model.__name__} {doc_id}: {str(e)}",
                exc_info=True,
            )
            raise
    ...
    def list_paginated(
        ...
        **filters,
        ...
        app_logger.info(
            f"Exiting list_paginated for {self.model.__name__} successfully: {len(items)}/{total} items"
        )
    ...
    def create(self, create_schema: TCreateSchema) -> TSchema:
        ...
        try:
            ...
            audit_logger.info(
                f"Audit: Created {self.model.__name__} with ID {doc_id} (org: {org_id})"
            )
            ...
            app_logger.warning(
                f"Conflict error creating {self.model.__name__}: {str(e)}"
            )
            raise ConflictError(
                f"{self.model.__name__} uniqueness constraint failed",
                details={"error": str(e)},
            )
        except MongoValidationError as e:
            app_logger.warning(
                f"DB constraint error creating {self.model.__name__}: {str(e)}"
            )
            raise ValidationError(
                f"Invalid data for {self.model.__name__}", details={"error": str(e)}
            )
        except Exception as e:
            error_logger.error(
    ...
    def update(
        ...
        app_logger.info(
            f"Entering update for {self.model.__name__} {doc_id} (org: {organization_id})"
        )
        try:
            filters = {"id": doc_id}
            if organization_id:
            ...
            if update_data:
                ...
                try:
                    ...
                    if "Trying to dereference unknown document" in str(reload_error):
                        ...
                        document = (
                            self.model.objects(**filters).no_dereference().first()
                        ) or document
                    else:
                        raise

            app_logger.info(
                f"Updated {self.model.__name__} {doc_id} (org: {organization_id})"
            )
            audit_logger.info(
                f"Audit: Updated {self.model.__name__} {doc_id} (org: {organization_id})"
            )

            result = self._to_schema(document)
            app_logger.debug(
                f"Exiting update for {self.model.__name__} {doc_id} successfully"
            )
            return result
        except (DoesNotExist, ValueError):
            ...
            app_logger.warning(
                f"Update caused a uniqueness conflict in {self.model.__name__} {doc_id}: {str(e)}"
            )
            raise ConflictError(
                f"Update caused a uniqueness conflict in {self.model.__name__}",
                details={"error": str(e)},
            )
        except MongoValidationError as e:
            app_logger.warning(
            ...
            error_logger.error(
                f"Error updating {self.model.__name__} {doc_id}: {str(e)}",
                exc_info=True,
            )
            raise
    ...
    def delete(
        ...
        try:
            ...
            if hasattr(document, "soft_delete") and not hard_delete:
                ...
                app_logger.info(
                    f"Soft deleted {self.model.__name__} {doc_id} (org: {organization_id})"
                )
                audit_logger.info(
                    f"Audit: Soft deleted {self.model.__name__} {doc_id} (org: {organization_id})"
                )
                return None

            document.delete()
            app_logger.warning(
                f"HARD deleted {self.model.__name__} {doc_id} (org: {organization_id})"
            )
            audit_logger.info(
                f"Audit: HARD deleted {self.model.__name__} {doc_id} (org: {organization_id})"
            )
            return None
        except (DoesNotExist, ValueError):
            app_logger.warning(
                f"{self.model.__name__} not found for deletion: {doc_id}"
            )
            raise NotFoundError(f"{self.model.__name__} not found for deletion")
        except Exception as e:
            error_logger.error(
                f"Error deleting {self.model.__name__} {doc_id}: {str(e)}",
                exc_info=True,
            )
            raise
