...
class EventReplayService:
    ...
    def replay_stream(self, topic: str, hours: int, organization_id: str = None) -> int:
        ...
        """
        app_logger.info(
            f"Initiating stream replay for topic: {topic}, hours: {hours}, org: {organization_id}"
        )
        audit_logger.info(
            f"Stream replay triggered: topic={topic}, hours={hours}, org_id={organization_id}"
        )
        ...
        try:
            ...
            app_logger.debug(
                f"Found {len(messages) if messages else 0} messages in range for replay"
            )
            ...
            for message_id, message_data in messages:
                ...
                msg_org_raw = self._message_get(
                    message_data, "organization_id", "unknown"
                )
            ...
            app_logger.info(
                f"Replay completed for {topic}. Processed {count} messages."
            )
        except Exception as e:
            error_logger.error(
                f"Error during replay of {topic}: {str(e)}", exc_info=True
            )

        return count

    def _dispatch_replay(
        self, topic: str, payload: Dict[str, Any], organization_id: str
    ):
    ...
    def retry_dlq(self, topic: str, organization_id: str = None) -> int:
        ...
        app_logger.info(
            f"Processing DLQ retry for topic: {topic}, org: {organization_id}"
        )
        audit_logger.info(
            f"DLQ retry triggered: topic={topic}, org_id={organization_id}"
        )
        ...
        app_logger.info(
            f"DLQ retry completed for {topic}. Reprocessed {count} messages."
        )
        return count
