...
from schemas.response import (
    FormResponseSchema,
)  # Reused generically here for DTO compliance
...
class AuditService:
    @staticmethod
    def append_event(
        tenant_id: str, actor_id: str, action: str, resource: str, metadata: dict = None
    ):
        """
        ...
        try:
            ...
            audit_logger.info(
                f"Audit event recorded: {action} on {resource} by {actor_id} in tenant {tenant_id}"
            )
