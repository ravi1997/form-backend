...
class HookService(BaseService):
    ...
    def trigger_question_hooks(
        ...
        app_logger.info(
            f"Triggering question hooks for form {form_id}, question {question_id}, user {user_id}"
        )
        try:
            form = Form.objects(
            ...
            if not form:
                app_logger.warning(
                    f"Form {form_id} not found for organization {organization_id}"
                )
            ...
            if not question:
                app_logger.warning(
                    f"Question {question_id} not found in form {form_id}"
                )
                raise NotFoundError("Question not found")

            results = self._execute_triggers(
                getattr(question, "logic", None), payload, organization_id
            )
            app_logger.info(f"Successfully triggered {len(results)} question hooks")
            return results
        except Exception as e:
            error_logger.error(
                f"Error triggering question hooks: {str(e)}", exc_info=True
            )
            raise

    def trigger_section_hooks(
        self,
        form_id: str,
        section_id: str,
        payload: Dict[str, Any],
        ...
        app_logger.info(
            f"Triggering section hooks for form {form_id}, section {section_id}, user {user_id}"
        )
        try:
            form = Form.objects(
                id=form_id, organization_id=organization_id, is_deleted=False
            ...
            results = self._execute_triggers(
                getattr(section, "logic", None), payload, organization_id
            )
            app_logger.info(f"Successfully triggered {len(results)} section hooks")
            return results
        except Exception as e:
            error_logger.error(
                f"Error triggering section hooks: {str(e)}", exc_info=True
            )
            raise

    def trigger_form_hooks(
        self, form_id: str, payload: Dict[str, Any], user_id: str, organization_id: str
    ) -> List[Dict[str, Any]]:
    ...
    def trigger_project_hooks(
        ...
        app_logger.info(
            f"Triggering project hooks for project {project_id}, user {user_id}"
        )
        try:
            ...
            if not project:
                app_logger.warning(
                    f"Project {project_id} not found for organization {organization_id}"
                )
            ...
            error_logger.error(
                f"Error triggering project hooks: {str(e)}", exc_info=True
            )
            raise

    def _execute_triggers(
        self, source: Any, payload: Dict[str, Any], organization_id: str
    ) -> List[Dict[str, Any]]:
        ...
        for trigger in source.triggers:
            ...
            app_logger.info(
                f"Executing trigger: {trigger.name} ({trigger.action_type})"
            )
    ...
    def _search_section_for_question(self, section, question_id):
        for question in getattr(section, "questions", []):
            if (
                str(getattr(question, "id", "")) == question_id
                or getattr(question, "variable_name", "") == question_id
            ):
                return question
    ...
    def _search_section_for_section(self, section, section_id):
        if (
            str(getattr(section, "id", "")) == section_id
            or getattr(section, "variable_name", "") == section_id
        ):
            return section
        for sub_section in getattr(section, "sections", []):
    ...
    def _execute_hook(self, trigger, payload, organization_id):
        ...
        try:
            ...
            app_logger.warning(
                f"Action type {action_type} not supported for synchronous execution"
            )
            return {
                "status": "ignored",
                "reason": f"Action type {action_type} not supported for synchronous execution",
            }
        except Exception as e:
            error_logger.error(
                f"Error executing hook {trigger.name}: {str(e)}", exc_info=True
            )
    ...
    def _handle_form_data_hook(self, config, payload, organization_id):
        ...
        return {
            "status": "success",
            "message": f"Synchronous data sync with form {target_form_id} completed",
        }
    ...
    def _handle_external_hook(self, config, payload, organization_id):
        ...
        if not hook_id:
            ...
            return {
                "status": "error",
                "message": "hook_id not specified in action_config",
            }

        hook = ExternalHook.objects(
            id=hook_id, organization_id=organization_id, is_deleted=False
        ).first()
        if not hook:
        ...
        if hook.status != "approved":
            app_logger.warning(
                f"External hook {hook_id} is not approved (status: {hook.status})"
            )
    ...
    def register_external_hook(self, data, user, organization_id):
        app_logger.info(
            f"Registering external hook {data.get('name')} for organization {organization_id}"
        )
        try:
            hook = ExternalHook(
                name=data.get("name"),
            ...
            hook.save()
            audit_logger.info(
                f"External hook registered: {hook.id} by user {user}",
                extra={
            ...
            error_logger.error(
                f"Error registering external hook: {str(e)}", exc_info=True
            )
            raise
    ...
    def approve_hook(self, hook_id, admin, status="approved"):
        ...
        try:
            ...
            audit_logger.info(
                f"External hook {hook_id} {status} by admin {admin}",
                extra={
            ...
            error_logger.error(
                f"Error approving hook {hook_id}: {str(e)}", exc_info=True
            )
            raise
