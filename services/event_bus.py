...
class EventBus:
    ...
    def publish(self, topic: str, payload: Dict[str, Any]):
        ...
        try:
                ...
                try:
                    if current_user:
                        organization_id = getattr(
                            current_user, "organization_id", "unknown"
                        )
                        envelope["organization_id"] = organization_id
                except Exception:
                    pass
            ...
            app_logger.info(
                f"Successfully streamed event to {topic}: {payload.get('id', 'unknown')}"
            )

            # Audit log significant events
            if topic in ["form.submitted", "tenant.key.rotated"]:
                audit_logger.info(
                    f"Event published: topic={topic}, event_id={envelope['event_id']}, org_id={organization_id}"
                )

        except Exception as e:
            error_logger.error(
                f"Failed to publish stream event {topic}: {str(e)}",
    ...
    def publish_to_dlq(self, topic: str, payload: Dict[str, Any], error_message: str):
        ...
        app_logger.warning(
            f"Publishing event to DLQ: {dlq_topic} due to: {error_message}"
        )
        try:
    ...
    def subscribe(
        ...
        app_logger.info(
            f"Initializing subscription for topic: {topic}, group: {group_name}"
        )

        # Ensure base stream and group exist, mask BUSYGROUP errors if already created
        try:
            redis_service.cache.client.xgroup_create(
                topic, group_name, id="0-0", mkstream=True
            )
        except Exception as e:
            if "BUSYGROUP" not in str(e):
                error_logger.error(
                    f"Error creating consumer group {group_name} on {topic}: {str(e)}"
                )

        app_logger.info(
            f"Started durable stream listener loop for {topic} (Group: {group_name})"
        )

        while True:
            try:
                # XREADGROUP pulls messages not yet ACKenowledged by this group, blocking up to 2s
                events = redis_service.cache.client.xreadgroup(
                    group_name, consumer_id, {topic: ">"}, count=10, block=2000
                )
                if not events:
                    # Periodically check for pending messages (un-acked)
                    self._process_pending_messages(
                        topic, group_name, consumer_id, handler, max_retries
                    )
                    continue
                ...
                for stream, messages in events:
                    for message_id, message_data in messages:
                        self._handle_message(
                            topic, group_name, message_id, message_data, handler
                        )

            except Exception as e:
                error_logger.error(
                    f"Stream polling error on topic {topic}: {str(e)}", exc_info=True
                )
    ...
    def _handle_message(
        ...
        try:
            ...
            app_logger.debug(
                f"Successfully acknowledged message {message_id} from {topic}"
            )
        except Exception as handler_error:
            error_logger.error(
                f"Stream handler error on {topic} for message {message_id}: {str(handler_error)}",
                exc_info=True,
            )
    ...
    def _process_pending_messages(
        ...
        try:
            ...
            min_idle_time = 60000
            messages = redis_service.cache.client.xautoclaim(
                topic, group_name, consumer_id, min_idle_time, start_id="0-0", count=10
            )

            if messages and len(messages) > 1 and messages[1]:
                app_logger.info(
                    f"Claimed {len(messages[1])} pending messages on {topic}"
                )
                for message_id, message_data in messages[1]:
                    # Check delivery count to prevent infinite retries
                    pel_item = redis_service.cache.client.xpending_range(
                        topic, group_name, message_id, message_id, 1
                    )
                    if pel_item:
                        delivery_count = pel_item[0].get("times_delivered") or pel_item[
                            0
                        ].get("deliv", 0)
                        if delivery_count > max_retries:
                            app_logger.warning(
                                f"Message {message_id} exceeded max retries ({delivery_count}). Moving to DLQ."
                            )
                            payload_raw = self._message_get(
                            ...
                            self.publish_to_dlq(
                                topic,
                                json.loads(payload_raw or "{}"),
                                "Max retries exceeded",
                            )
                            redis_service.cache.client.xack(
                                topic, group_name, message_id
                            )
                            continue
            ...
            error_logger.error(
                f"Error processing pending messages on {topic}: {str(e)}", exc_info=True
            )
