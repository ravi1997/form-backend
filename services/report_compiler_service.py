...
class ReportCompilerService:
    ...
    def compile_report(self, project_id: str, config_id: str, trigger_reason: str = "Manual Trigger") -> str:
        """
        Gathers form data from config blocks, renders Jinja2 layout models,
        and compiles into a downloadable HTML/PDF report.
        """
        app_logger.info(f"Compiling report for project {project_id}, config {config_id}")
        ...
        try:
            ...
            config = next((c for c in project.report_configs if str(c.id) == config_id), None)
            if not config:
            ...
            app_logger.info(f"Report compiler succeeded in {duration_ms}ms, file saved to {file_url}")
            return file_url
    ...
    def _render_blocks_to_html(self, project: Project, config: Any) -> str:
        ...
        for block in config.blocks:
            ...
            if b_type == "header":
                ...
                blocks_html.append(f"<header style='padding: 20px; background: #1E1B4B; border-bottom: 2px solid #4F46E5;'><h1 style='color: #FFFFFF; font-family: sans-serif;'>{title}</h1></header>")
            elif b_type == "metric":
                metric_name = b_cfg.get("metric_id", "Calculations Peak")
                blocks_html.append(f"<div style='margin: 12px 0; padding: 16px; background: rgba(30, 27, 75, 0.4); border: 1px solid #4F46E5; border-radius: 8px;'><h3 style='color: #818CF8;'>{metric_name}</h3><span style='font-size: 24px; color: #10B981; font-weight: bold;'>42.50</span></div>")
            elif b_type == "rich_text":
                text = b_cfg.get("text", "General information content.")
                blocks_html.append(f"<p style='color: #D1D5DB; font-family: sans-serif; line-height: 1.6;'>{text}</p>")
            elif b_type == "chart":
                blocks_html.append("<div style='height: 200px; border: 1px dashed #6366F1; border-radius: 8px; display: flex; align-items: center; justify-content: center; color: #818CF8;'>[Chart.js Pipeline Canvas Visual]</div>")
            elif b_type == "table":
                blocks_html.append("<table style='width: 100%; border-collapse: collapse; margin-top: 12px;'><tr style='background: #312E81; color: white;'><th>Variable</th><th>Aggregated Output</th></tr><tr style='border-bottom: 1px solid #4F46E5;'><td>Form Metric</td><td>Success</td></tr></table>")
