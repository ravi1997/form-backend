...
def _deep_clone_section(original_section):
    ...
    if original_section.sections:
        new_section.sections = [
            _deep_clone_section(s) for s in original_section.sections
        ]
...
def async_clone_form(
    self, form_id, user_id, organization_id, new_title=None, new_slug=None
):
    """
    Background task to clone a form with full deep-copy of sections.
    """
    app_logger.info(
        f"Entering async_clone_form: form_id={form_id}, user_id={user_id}, organization_id={organization_id}"
    )
    try:
        ...
        if original.organization_id != organization_id:
            error_logger.error(
                f"Tenant violation in async_clone_form: Form {form_id} org {original.organization_id} vs user org {organization_id}"
            )
        ...
        audit_logger.info(
            f"Form {form_id} cloned to {new_form.id} via background task. Action by User {user_id} for Org {organization_id}"
        )
        app_logger.info(
            f"Exiting async_clone_form: successfully cloned {form_id} to {new_form.id}"
        )
        return {"status": "success", "form_id": str(new_form.id), "slug": final_slug}

    except Exception as e:
        error_logger.error(
            f"Task async_clone_form failed for {form_id}: {str(e)}", exc_info=True
        )
        raise self.retry(exc=e)


@celery_app.task(bind=True, max_retries=2, default_retry_delay=60)
def async_publish_form(
    self, form_id, organization_id, major_bump=False, minor_bump=True
):
    """
    Background task to publish a form (semantic versioning and snapshotting).
    """
    app_logger.info(
        f"Entering async_publish_form: form_id={form_id}, organization_id={organization_id}"
    )
    try:
...
def async_bulk_export(self, job_id, organization_id):
    ...
    app_logger.info(
        f"Entering async_bulk_export: job_id={job_id}, organization_id={organization_id}"
    )
    ...
    try:
            ...
            for idx, fid in enumerate(job.form_ids):
                try:
                    ...
                    responses = FormResponse.objects(
                        form=form.id, organization_id=organization_id
                    ).no_cache()
                    ...
                    app_logger.warning(
                        f"Failed to export form {fid} in bulk job {job_id}: {e}"
                    )
        ...
        app_logger.info(
            f"BulkExport job {job_id} completed successfully. Exported {exported_count} forms."
        )
...
def async_recalculate_materialized_view(self, view_id, organization_id):
    ...
    app_logger.info(
        f"Entering async_recalculate_materialized_view: view_id={view_id}, organization_id={organization_id}"
    )
    try:
        service = DynamicViewService()
        view_def = service.model.objects(
            id=view_id, organization_id=organization_id, is_deleted=False
        ).first()
        if not view_def:
            error_logger.error(
                f"View {view_id} not found for recalculation. Org: {organization_id}"
            )
        ...
        audit_logger.info(
            f"View {view_id} recalculated and snapshot {snapshot.id} created via background task. Org: {organization_id}"
        )
        app_logger.info(
            f"Exiting async_recalculate_materialized_view: successfully recalculated {view_id}"
        )
        return {"status": "success", "snapshot_id": str(snapshot.id)}

    except Exception as e:
        error_logger.error(
            f"Task async_recalculate_materialized_view failed for {view_id}: {str(e)}",
            exc_info=True,
        )
        raise self.retry(exc=e)
    ...
    try:
        ...
        for i, lang in enumerate(job.target_languages):
            if job.reload().status == "cancelled":
                app_logger.info(
                    f"Job {job_id} cancelled during processing at language {lang}"
                )
                break

            try:
                app_logger.info(
                    f"Translating form {job.form_id} to {lang} (Job: {job_id})"
                )
                ...
                error_logger.error(
                    f"Error translating form {job.form_id} to {lang}: {str(e)}"
                )
                results[lang] = {
                    "success": False,
                    "success_count": 0,
        ...
        try:
            ...
            pass
        error_logger.error(
            f"Error processing translation job {job_id}: {str(e)}", exc_info=True
        )
        raise self.retry(exc=e)


@celery_app.task(bind=True, max_retries=3, default_retry_delay=3600)
def cleanup_deleted_records(self, retention_days: int = 30, dry_run: bool = False):
    """
    GDPR Compliance: Permanently delete records that have been soft-deleted for longer than the retention period.
    ...
    Args:
        ...
        dry_run: If True, only count records without actually deleting them (default: False)
    ...
    app_logger.info(
        f"Starting GDPR cleanup: retention_days={retention_days}, dry_run={dry_run}"
    )
    ...
    try:
        ...
        deleted_forms = []
        forms_query = Form.objects(is_deleted=True)

        for form in forms_query:
            if hasattr(form, "deleted_at") and form.deleted_at:
                if form.deleted_at < cutoff_date:
                    if not dry_run:
                        app_logger.info(
                    ...
                    deleted_forms.append(str(form.id))

        # Count and delete soft-deleted FormResponses
        deleted_responses = []
        responses_query = FormResponse.objects(is_deleted=True)

        for response in responses_query:
            if hasattr(response, "deleted_at") and response.deleted_at:
                if response.deleted_at < cutoff_date:
                    if not dry_run:
                        app_logger.info(
                    ...
                    deleted_responses.append(str(response.id))
        ...
        result = {
            ...
            "deleted_forms_count": len(deleted_forms),
            "deleted_forms": deleted_forms,
            "deleted_responses_count": len(deleted_responses),
            "deleted_responses": deleted_responses,
            "total_deleted": len(deleted_forms) + len(deleted_responses),
        }

        audit_logger.info(
            f"GDPR cleanup completed: {result['total_deleted']} records {'would be ' if dry_run else ''}permanently deleted. "
            f"Forms: {result['deleted_forms_count']}, Responses: {result['deleted_responses_count']}"
        )
        ...
        error_logger.error(f"GDPR cleanup failed: {str(e)}", exc_info=True)
        raise self.retry(exc=e)
