...
@swag_from(
    {
        ...
        "responses": {
            "200": {
                "description": "Return currently authenticated user's profile.",
...
def get_profile():
    ...
    safe_log_info(app_logger, "User %s fetching profile", str(current_user.id))
...
@swag_from(
    {
        ...
        "responses": {
            "200": {
                "description": "Securely change current user's password.",
                "schema": {"$ref": "#/definitions/UserOut"},
            }
        },
    }
...
def get_user_by_id(user_id):
    ...
    try:
        ...
        return success_response(
            data=UserOut.model_validate(user.model_dump()).model_dump()
        )
        ...
        error_logger.error(
            f"Error fetching user {user_id} by admin {admin_id}: {str(e)}"
        )
        ...
        "parameters": [
            {
                "name": "body",
                "in": "body",
                "schema": {"$ref": "#/definitions/UserUpdateSchema"},
            }
        ],
    }
)
...
def create_user():
    ...
    try:
        ...
        error_logger.warning(
            f"Admin user creation failed (Admin: {admin_id}): {str(e)}"
        )
...
def update_user_by_id(user_id):
    ...
    try:
        ...
        error_logger.error(
            f"User update failed for {user_id} (Admin: {admin_id}): {str(e)}"
        )
        return error_response(message=str(e), status_code=400)
...
@swag_from(
    {
        "tags": ["User"],
        "responses": {
            "200": {"description": "Soft-delete a user account. Superadmin only."}
        },
        "parameters": [
            {"name": "user_id", "in": "path", "type": "string", "required": True}
        ],
...
def delete_user_by_id(user_id):
    ...
    try:
        ...
        error_logger.error(
            f"User deactivation failed for {user_id} (Superadmin: {admin_id}): {str(e)}"
        )
        raise
...
def update_user_roles(user_id):
    ...
    try:
        ...
        error_logger.error(
            f"Role update failed for {user_id} (Admin: {admin_id}): {str(e)}"
        )
        return error_response(message=str(e), status_code=400)
...
@swag_from(
    {
        "tags": ["User"],
        "responses": {
            "200": {"description": "Manually lock a user account. Admin only."}
        },
        "parameters": [
            {"name": "user_id", "in": "path", "type": "string", "required": True}
        ],
...
def lock_user_account(user_id):
    ...
    try:
        ...
        error_logger.error(
            f"Account lock failed for {user_id} (Admin: {admin_id}): {str(e)}"
        )
        raise
...
@swag_from(
    {
        "tags": ["User"],
        "responses": {
            "200": {"description": "Manually unlock a user account. Admin only."}
        },
        "parameters": [
            {"name": "user_id", "in": "path", "type": "string", "required": True}
        ],
...
def unlock_user_account(user_id):
    ...
    try:
        ...
        user.unlock_account()
        audit_logger.info(
            f"User {user_id} account manually unlocked by admin {admin_id}"
        )
        return success_response(message=f"User {user_id} account unlocked")
    except Exception as e:
        error_logger.error(
            f"Account unlock failed for {user_id} (Admin: {admin_id}): {str(e)}"
        )
        raise
