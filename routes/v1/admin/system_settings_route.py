...
@swag_from(
    {
        ...
        "responses": {
            "200": {"description": "Retrieve the global system configuration."}
        },
    }
)
...
def get_system_settings():
    ...
    try:
        ...
        return error_response(
            message="Failed to retrieve configuration", status_code=500
        )
...
@swag_from(
    {
        "tags": ["System_Settings"],
        "responses": {
            "200": {"description": "Update the global system configuration."}
        },
        "parameters": [
            {
                "name": "body",
                "in": "body",
                "schema": {"$ref": "#/definitions/SystemSettingsUpdateSchema"},
            }
        ],
    }
)
...
def update_system_settings():
    ...
    if not data:
        app_logger.warning(
            f"update_system_settings failed: no data provided by admin: {admin_id}"
        )
    ...
    try:
        ...
        audit_logger.info(
            f"System settings updated by admin {admin_id}. Updated fields: {list(data.keys())}"
        )
        app_logger.info(
            f"Exiting update_system_settings successfully by admin: {admin_id}"
        )
        return success_response(data=result.model_dump())
    except Exception as e:
