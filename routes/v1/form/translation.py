...
@swag_from({"tags": ["Translation"], "responses": {"200": {"description": "Success"}}})
@jwt_required()
def get_translations():
    ...
    try:
        ...
        if not has_form_permission(current_user, form, "view"):
            app_logger.warning(
                f"Unauthorized access attempt to translations for form {form_id} by user {current_user.id}"
            )
            return error_response(message="Unauthorized", status_code=403)

        if not form.versions:
            app_logger.info(
                f"Form {form_id} has no versions, returning empty translations"
            )
        ...
        error_logger.error(
            f"Error in get_translations for form {form_id}: {str(e)}", exc_info=True
        )
...
def save_translations():
    ...
    if not form_id or not language or translations is None:
        ...
        return error_response(
            message="Missing form_id, language, or translations", status_code=400
        )
    ...
    try:
        ...
        if not has_form_permission(current_user, form, "edit"):
            app_logger.warning(
                f"Unauthorized attempt to save translations for form {form_id} by user {current_user.id}"
            )
            return error_response(message="Unauthorized", status_code=403)

        if not form.versions:
            app_logger.warning(
                f"Attempt to save translations for form {form_id} which has no versions"
            )
        ...
        audit_logger.info(
            f"User {current_user.id} saved translations for form {form_id} and language {language}",
            extra={
        ...
        error_logger.error(
            f"Error in save_translations for form {form_id}: {str(e)}", exc_info=True
        )
        return error_response(message=str(e), status_code=400)
...
def preview_translation():
    ...
    if not text or not target_lang:
        ...
        return error_response(
            message="Missing text or target_language", status_code=400
        )
...
def handle_jobs():
    if request.method == "GET":
        ...
        try:
            ...
            return success_response(
                data=[
                    BaseSerializer.clean_dict(job.to_mongo().to_dict()) for job in jobs
                ]
            )
        except Exception as e:
            error_logger.error(
                f"Error in handle_jobs (GET) for form {form_id}: {str(e)}",
                exc_info=True,
            )
    ...
    if not form_id or not target_languages:
        ...
        return error_response(
            message="Missing form_id or target_languages", status_code=400
        )
    ...
    try:
        ...
        if not has_form_permission(current_user, form, "edit"):
            app_logger.warning(
                f"Unauthorized attempt to start translation job for form {form_id} by user {current_user.id}"
            )
            return error_response(message="Unauthorized", status_code=403)
        ...
        audit_logger.info(
            f"User {current_user.id} started translation job {job.id} for form {form_id}",
            extra={
                "user_id": str(current_user.id),
                "form_id": form_id,
                "job_id": str(job.id),
        ...
        app_logger.info(
            f"Exiting handle_jobs (POST) for form_id: {form_id}, job_id: {job.id}"
        )
        return success_response(
        ...
        error_logger.error(
            f"Error starting translation job for form {form_id}: {str(e)}",
            exc_info=True,
        )
        return error_response(message=str(e), status_code=400)
...
def get_job_status(job_id):
    ...
    try:
        ...
        return success_response(
            data=BaseSerializer.clean_dict(job.to_mongo().to_dict())
        )
    except Exception as e:
...
def cancel_job(job_id):
    ...
    try:
        ...
        if job.status in ["pending", "in_progress"]:
            ...
            audit_logger.info(
                f"User {current_user.id} cancelled translation job {job_id}",
                extra={
                    "user_id": str(current_user.id),
                    "job_id": job_id,
        ...
        return error_response(
            message=f"Cannot cancel job in {job.status} state", status_code=400
        )
    except Exception as e:
...
def delete_job(job_id):
    ...
    try:
        ...
        audit_logger.info(
            f"User {current_user.id} deleted translation job {job_id}",
            extra={
                "user_id": str(current_user.id),
                "job_id": job_id,
...
def get_translated_content(job_id):
    ...
    try:
        ...
        if job.status != "completed":
            app_logger.warning(
                f"Attempt to fetch content for incomplete job {job_id} (status: {job.status})"
            )
        ...
        return success_response(
            data={"form_id": str(job.form_id), "results": job.results}
        )
    except Exception as e:
        app_logger.warning(f"Translation job not found: {job_id}")
        return error_response(message="Job not found", status_code=404)
