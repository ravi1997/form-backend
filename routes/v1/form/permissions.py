...
def get_form_permissions(project_id, form_id):
    ...
    try:
        ...
        if not has_form_permission(current_user, form, "edit"):
            app_logger.warning(
                f"Unauthorized access attempt to permissions for form {form_id} by user {current_user.id}"
            )
...
def update_form_permissions(project_id, form_id):
    ...
    try:
        ...
        if not has_form_permission(current_user, form, "edit"):
            app_logger.warning(
                f"Unauthorized attempt to update permissions for form {form_id} by user {current_user.id}"
            )
            return jsonify({"error": "Unauthorized"}), 403
        ...
        audit_logger.info(
            f"User {current_user.id} updated permissions for form {form_id}",
            extra={
