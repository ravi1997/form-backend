...
def summarize(form_id: str):
    ...
    try:
        ...
        if response_ids:
            responses_count = FormResponse.objects(
                id__in=response_ids, form=form_uuid, is_deleted=False
            ).count()
        else:
            responses_count = FormResponse.objects(
                form=form_uuid, is_deleted=False
            ).count()

        if responses_count < 2:
            app_logger.warning(
                f"Insufficient responses for summarization for form {form_id}: {responses_count}"
            )
            return (
                jsonify(
                    {
        ...
        summary_text = summarizer.summarize_form(
            str(form.id), response_ids=response_ids
        )

        app_logger.info(f"Exiting summarize for form_id: {form_id}")
        return (
            jsonify(
                {"success": True, "summary": summary_text, "form_id": str(form.id)}
            ),
            200,
        )
    except Exception as e:
        error_logger.error(
            f"Summarization error for form {form_id}: {e}", exc_info=True
        )
...
def summarize_stream(form_id: str):
    ...
    def generate():
        try:
            ...
            error_logger.error(
                f"Summarization streaming error for form {form_id}: {e}", exc_info=True
            )
