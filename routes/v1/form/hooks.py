...
@swag_from(
    {
        ...
        "responses": {"200": {"description": "Execution results for all hooks"}},
    }
)
@jwt_required()
def trigger_question_hooks(form_id, question_id):
    app_logger.info(
        f"Entering trigger_question_hooks for form {form_id}, question {question_id}"
    )
    ...
    try:
        ...
        audit_logger.info(
            f"Question hooks triggered for question {question_id} by user {current_user.id}"
        )
...
def trigger_section_hooks(form_id, section_id):
    app_logger.info(
        f"Entering trigger_section_hooks for form {form_id}, section {section_id}"
    )
    current_user = get_current_user()
    payload = request.get_json(silent=True) or {}
    try:
        ...
        audit_logger.info(
            f"Section hooks triggered for section {section_id} by user {current_user.id}"
        )
        return jsonify({"results": results}), 200
    except Exception as e:
...
def trigger_form_hooks(form_id):
    ...
    try:
        ...
        audit_logger.info(
            f"Form hooks triggered for form {form_id} by user {current_user.id}"
        )
        return jsonify({"results": results}), 200
    except Exception as e:
...
def trigger_project_hooks(project_id):
    ...
    try:
        ...
        audit_logger.info(
            f"Project hooks triggered for project {project_id} by user {current_user.id}"
        )
        return jsonify({"results": results}), 200
    except Exception as e:
...
@swag_from(
    {
        ...
        "responses": {"201": {"description": "Hook registered and pending approval"}},
    }
)
@jwt_required()
def register_hook():
    ...
    try:
        hook = hook_service.register_external_hook(
            data=data, user=current_user, organization_id=current_user.organization_id
        )
        audit_logger.info(
            f"External hook {hook.id} registered by user {current_user.id}"
        )
        return (
            jsonify(
                {
                    "message": "Hook registered and pending approval",
                    "hook_id": str(hook.id),
                }
            ),
            201,
...
@swag_from(
    {
        ...
        "responses": {"200": {"description": "Hook status updated"}},
    }
)
@jwt_required()
...
def approve_hook(hook_id):
    ...
    try:
        hook = hook_service.approve_hook(
            hook_id=hook_id, admin=current_user, status=status
        )
