...
def has_form_permission(user, form, action):
    ...
    if str(form.created_by) == user_id_str:
        safe_log_info(
            app_logger, "User %s is creator, permission granted", str(user.id)
        )
        return True
    ...
    if action == "view":
        if (
            ...
            app_logger.info(
                f"View permission granted for user {user.id} via viewers/editors/public"
            )
            return True
        if policy:
            if policy.form_visibility == "public":
                app_logger.info(
                    f"View permission granted for user {user.id} via public policy"
                )
                return True
            if policy.form_visibility == "restricted":
                if user_dept and user_dept in (policy.allowed_departments or []):
                    app_logger.info(
                        f"View permission granted for user {user.id} via department {user_dept}"
                    )
                    return True
                if is_in_list(policy.can_view_responses):
                    app_logger.info(
                        f"View permission granted for user {user.id} via can_view_responses list"
                    )
                    return True
    ...
    if action in ("edit", "edit_design"):
        if user_id_str in (form.editors or []):
            app_logger.info(
                f"Edit permission granted for user {user.id} via editors list"
            )
            return True
        if policy and is_in_list(policy.can_edit_design):
            app_logger.info(
                f"Edit permission granted for user {user.id} via policy can_edit_design"
            )
            return True
    ...
    if action == "manage_access":
        if hasattr(user, "is_admin_check") and user.is_admin_check():
            app_logger.info(
                f"Manage access permission granted for user {user.id} via admin check"
            )
            return True
        if policy and is_in_list(policy.can_manage_access):
            app_logger.info(
                f"Manage access permission granted for user {user.id} via policy can_manage_access"
            )
            return True
    ...
    if action == "view_responses":
        if user_id_str in (form.editors or []):
            app_logger.info(
                f"View responses permission granted for user {user.id} via editors list"
            )
            return True
        if policy and is_in_list(policy.can_view_responses):
            app_logger.info(
                f"View responses permission granted for user {user.id} via policy can_view_responses"
            )
            return True
    ...
    if action == "edit_responses":
        if user_id_str in (form.editors or []):
            app_logger.info(
                f"Edit responses permission granted for user {user.id} via editors list"
            )
            return True
        if policy and is_in_list(policy.can_edit_responses):
            app_logger.info(
                f"Edit responses permission granted for user {user.id} via policy can_edit_responses"
            )
            return True
    ...
    if action == "delete_responses":
        if hasattr(user, "is_admin_check") and user.is_admin_check():
            app_logger.info(
                f"Delete responses permission granted for user {user.id} via admin check"
            )
            return True
        if policy and is_in_list(policy.can_delete_responses):
            app_logger.info(
                f"Delete responses permission granted for user {user.id} via policy can_delete_responses"
            )
            return True
    ...
    if action == "view_audit":
        if user_id_str in (form.editors or []):
            app_logger.info(
                f"View audit permission granted for user {user.id} via editors list"
            )
            return True
        if policy and is_in_list(policy.can_view_audit_logs):
            app_logger.info(
                f"View audit permission granted for user {user.id} via policy can_view_audit_logs"
            )
            return True
    ...
    if action == "delete_form":
        if hasattr(user, "is_admin_check") and user.is_admin_check():
            app_logger.info(
                f"Delete form permission granted for user {user.id} via admin check"
            )
            return True
        if policy and is_in_list(policy.can_delete_form):
            app_logger.info(
                f"Delete form permission granted for user {user.id} via policy can_delete_form"
            )
            return True
    ...
    def translate_sections(sections):
        ...
        for section in sections:
            ...
            if sid in section_translations:
                ...
                if "title" in s_trans:
                    section["title"] = s_trans["title"]
                if "description" in s_trans:
                    section["description"] = s_trans["description"]
            ...
            for question in section.get("questions", []):
                ...
                q_trans = question_translations.get(
                    var_name
                ) or question_translations.get(qid)
                ...
                if q_trans:
                    ...
                    if "options" in q_trans and "options" in question:
                        ...
                        for option in question.get("options", []):
                            ...
                            o_trans = option_translations.get(
                                ov
                            ) or option_translations.get(oid)
                            if o_trans:
