...
def share_form(form_id):
    ...
    try:
        ...
        # Allow public forms to be accessed by different organizations
        form = Form.objects.get(id=form_id, is_deleted=False)

        # Check organization match for private forms
        if not form.is_public and form.organization_id != current_user.organization_id:
            app_logger.warning(
                f"Cross-tenant access attempt to form {form_id} by user {current_user.id} "
                f"(user org: {current_user.organization_id}, form org: {form.organization_id})"
            )
...
@swag_from(
    {
        "tags": ["Form"],
        "responses": {
            "200": {"description": "Admin only: Change form status to 'archived'."}
        },
        "parameters": [
            {"name": "form_id", "in": "path", "type": "string", "required": True}
        ],
...
def restore_form(form_id):
    ...
    try:
        ...
        form = Form.objects.get(
            id=form_id, organization_id=current_user.organization_id, status="archived"
        )
    ...
    try:
        ...
        if data.get("confirm") != "DELETE_ALL":
            return error_response(
                message="Confirmation required inside JSON body {'confirm': 'DELETE_ALL'}",
                status_code=400,
            )
...
def toggle_form_public(form_id):
    ...
    try:
        ...
        return success_response(
            data={"is_public": form.is_public}, message="Form public access toggled"
        )
    except DoesNotExist:
        error_logger.warning(
        ...
        error_logger.error(f"Error toggling public access for form {form_id}: {str(e)}")
        return error_response(message="Internal server error", status_code=500)
...
@swag_from(
    {
        "tags": ["Form"],
        "responses": {"200": {"description": "Get total submission count for a form."}},
        "parameters": [
            {"name": "form_id", "in": "path", "type": "string", "required": True}
        ],
    }
...
@swag_from(
    {
        "tags": ["Form"],
        "responses": {
            "200": {"description": "Fetch the most recent response record for a form."}
        },
        "parameters": [
            {"name": "form_id", "in": "path", "type": "string", "required": True}
        ],
...
def last_response(form_id):
    ...
    try:
        ...
        error_logger.error(f"Error fetching last response for form {form_id}: {str(e)}")
        return error_response(message="Internal server error", status_code=500)
