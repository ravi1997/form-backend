...
def validate_form_submission(
    form, submitted_data, logger=None, is_draft=False, organization_id=None
):
    """
    ...
    mapped_errors = []
    for err in errors:
        mapped_errors.append({"id": err.get("field"), "error": err.get("error")})
...
def evaluate_conditions_endpoint():
    ...
    try:
        ...
        app_logger.info(
            f"Exiting evaluate_conditions_endpoint: {len(results)} conditions evaluated"
        )
