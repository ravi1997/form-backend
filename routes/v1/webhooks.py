...
def get_webhook_status(delivery_id: str):
    ...
    app_logger.info(f"Fetching webhook status for delivery_id: {delivery_id}")
    status = WebhookService.get_webhook_status(delivery_id)
    if not status:
        app_logger.warning(f"Webhook delivery not found: {delivery_id}")
        return error_response(message="Webhook delivery not found", status_code=404)
...
def get_webhook_history(delivery_id=None):
    ...
    app_logger.info(f"Fetching webhook history. delivery_id: {delivery_id}")
...
def test_webhook():
    ...
    admin_id = get_jwt_identity()
    app_logger.info(f"Test webhook by admin: {admin_id}")
    data = request.get_json(silent=True) or {}
    url = data.get("url")
    payload = data.get("payload")
...
def get_webhook_logs():
    ...
    app_logger.info("Fetching webhook logs")
    url = request.args.get("url")
    status = request.args.get("status")
    limit = min(request.args.get("limit", 100, type=int), 500)
    logs = WebhookService.get_webhook_logs(url=url, status=status, limit=limit)
    app_logger.info(f"Webhook logs fetched. Count: {len(logs)}")
