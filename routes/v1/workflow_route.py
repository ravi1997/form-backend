...
def create_workflow():
    ...
    try:
        ...
        try:
            ...
            app_logger.warning(
                f"create_workflow failed: invalid trigger_form_id format: {trigger_form_value}"
            )
        ...
        if not trigger_form:
            app_logger.warning(
                f"create_workflow failed: trigger form {trigger_form_uuid} not found for org {current_user.organization_id}"
            )
            return error_response(
        ...
        app_logger.info(
            f"Exiting create_workflow successfully. Workflow ID: {workflow.id}"
        )
...
def list_workflows():
    ...
    try:
        ...
        app_logger.info(
            f"Exiting list_workflows successfully. Found {len(result)} workflows."
        )
...
def get_workflow(workflow_id):
    ...
    if not HAS_WORKFLOW_MODEL:
        app_logger.warning(
            f"get_workflow failed for {workflow_id}: workflow models not available"
        )
        return _no_model()
    ...
    try:
        try:
            ...
            app_logger.warning(
                f"get_workflow failed: invalid workflow_id format: {workflow_id}"
            )
        ...
        if not workflow:
            app_logger.warning(
                f"get_workflow failed: workflow {workflow_uuid} not found for org {current_user.organization_id}"
            )
            return error_response("Workflow not found", status_code=404)

        app_logger.info(
            f"Exiting get_workflow successfully for workflow_id: {workflow_id}"
        )
...
def list_pending_approvals():
    ...
    try:
        ...
        app_logger.info(
            f"Exiting list_pending_approvals successfully. Found {len(pending_dicts)} pending approvals."
        )
        return success_response(
            data={"items": pending_dicts, "total": len(pending_dicts)}
        )
    except Exception as e:
...
def update_workflow(workflow_id):
    ...
    if not HAS_WORKFLOW_MODEL:
        app_logger.warning(
            f"update_workflow failed for {workflow_id}: workflow models not available"
        )
        return _no_model()
    ...
    try:
        try:
            ...
            app_logger.warning(
                f"update_workflow failed: invalid workflow_id format: {workflow_id}"
            )
            return error_response("Invalid workflow_id format", status_code=400)
        ...
        if not workflow:
            app_logger.warning(
                f"update_workflow failed: workflow {workflow_uuid} not found for org {current_user.organization_id}"
            )
            return error_response("Workflow not found", status_code=404)
        ...
        if "steps" in data:
            workflow.steps = [
                _frontend_step_to_backend(s_data) for s_data in data["steps"]
            ]

        workflow.save()
        audit_logger.info(
            f"Workflow updated. ID: {workflow.id}, Name: {workflow.name}, Org: {current_user.organization_id}, User: {current_user.id}"
        )
        app_logger.info(
            f"Exiting update_workflow successfully for workflow_id: {workflow_id}"
        )
        return success_response(
...
def delete_workflow(workflow_id):
    ...
    if not HAS_WORKFLOW_MODEL:
        app_logger.warning(
            f"delete_workflow failed for {workflow_id}: workflow models not available"
        )
        return _no_model()
    ...
    try:
        try:
            ...
            app_logger.warning(
                f"delete_workflow failed: invalid workflow_id format: {workflow_id}"
            )
            return error_response("Invalid workflow_id format", status_code=400)
        ...
        if not workflow:
            app_logger.warning(
                f"delete_workflow failed: workflow {workflow_uuid} not found for org {current_user.organization_id}"
            )
            return error_response("Workflow not found", status_code=404)
        ...
        app_logger.info(
            f"Exiting delete_workflow successfully for workflow_id: {workflow_id}"
        )
