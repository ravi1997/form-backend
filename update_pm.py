...
def fix_request(item, path=""):
    ...
    if method in ["POST", "PUT", "PATCH"]:
        if (
            ...
            # Dashboards & Widgets
                ...
                new_body = {
                    "positions": {"{{widget_id}}": {"x": 0, "y": 0, "w": 2, "h": 2}}
                }
            ...
            # AI
                ...
                new_body = {"query": "quality complaints", "results_count": 5}
            elif "ai/forms/:form_id/executive-summary" in url:
                new_body = {"audience": "leadership", "tone": "formal", "max_points": 5}
                ...
                new_body = {
                    "prompt": "Generate a patient feedback form",
                    "industry": "healthcare",
                }
    ...
    if "auth/register" in url:
        pre_script.append(
            'pm.environment.set("created_email", "test.user." + Date.now() + "@example.com");'
        )
    elif method == "POST" and "forms" in url and ":form_id" not in url:
        pre_script.append('pm.environment.set("form_slug", "form-" + Date.now());')

    if req.get("auth") or (
        req.get("header")
        and any(h.get("key") == "Authorization" for h in req.get("header"))
    ):
        pre_script.append(
            'if (!pm.environment.get("access_token")) { console.warn("Missing access_token!"); }'
        )
