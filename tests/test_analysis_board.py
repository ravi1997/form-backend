...
def test_topological_sorting(analysis_service):
    ...
    circular_order = analysis_service._resolve_topological_order([node_a, node_b, node_c])
...
def test_execute_aspect_nodes(analysis_service):
    ...
    # Mock inputs results
    resolved_parent_results = {
        str(node.inputs[0]): 100.0,
        str(node.inputs[1]): 5.0
    }

    res = analysis_service._execute_single_node(node, resolved_parent_results, "org-123")
    assert res == 20.0
...
def test_crud_endpoints(app):
    ...
    try:
        app.register_blueprint(
            analysis_board_bp,
            url_prefix="/form/api/v1/projects/<project_id>/analysis-boards"
        )
    ...
    with app.app_context():
        ...
        with patch("utils.security_helpers.get_current_user", return_value=mock_user), \
             patch.object(Project, "objects") as mock_project_objects, \
             patch("routes.v1.analysis_board_route.analysis_board_service") as mock_service, \
             patch("utils.security_helpers.AccessControlService.check_project_permission", return_value=True):
            ...
            resp = client.post(
                f"/form/api/v1/projects/{project_id}/analysis-boards/",
                json=payload,
                headers=headers
            )
            ...
            resp = client.get(
                f"/form/api/v1/projects/{project_id}/analysis-boards/board-123",
                headers=headers
            )
