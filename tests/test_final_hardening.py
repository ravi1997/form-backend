...
def test_aggregate_repeat_calculations():
    ...
    with patch.object(Form, "objects") as mock_form_objs, patch.object(
        ...
        payload = {"members": [{"age": 10}, {"age": 20}, {"age": 5}]}

        valid, cleaned, errors, calc = FormValidationService.validate_submission(
            form_id="form1", payload=payload, organization_id="org1"
        )
...
def test_global_tenant_isolation_queryset():
    ...
    with app.app_context():
        with patch("models.base.current_user", mock_user), patch(
            "models.base.has_request_context", return_value=True
