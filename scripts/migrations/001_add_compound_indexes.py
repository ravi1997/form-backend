...
def _create_indexes(collection, indexes, label):
    ...
    for idx in indexes:
        try:
            ...
            if "already exists" in str(e).lower() or "IndexOptionsConflict" in str(
                type(e)
            ):
                logger.info(
                    f"  ⏭  {label}: {idx.document.get('name', '?')} (already exists)"
                )
            else:
...
def run():
    ...
    if result.modified_count:
        logger.info(
            f"  ✅ Backfilled {result.modified_count} snapshot docs with empty organization_id"
        )
    else:
