    ...
    while i < len(lines):
        ...
        if match:
            ...
            if "methods=" in route_params:
                if "POST" in route_params:
                    method = "POST"
                elif "PUT" in route_params:
                    method = "PUT"
                elif "DELETE" in route_params:
                    method = "DELETE"
            ...
            path_params = re.findall(r"<[^>]+:([^>]+)>", route_params) or re.findall(
                r"<([^>]+)>", route_params
            )
            ...
            swagger_dict = {"tags": [tag_name], "responses": responses}
            if parameters:
            ...
            if found_existing_idx != -1:
                ...
                if "swag_from({" in lines[found_existing_idx]:
                    for j in range(
                        found_existing_idx, min(found_existing_idx + 20, len(lines))
                    ):
                        if "})" in lines[j]:
                            end_idx = j
                            break
