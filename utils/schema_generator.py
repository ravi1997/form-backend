...
from typing import Dict, Any, Type, Optional, List, Annotated
...
def date_validator(v: Any, validation: Any) -> Any:
    ...
    try:
        ...
        if isinstance(e, ValueError) and str(e) in [
            ...
            f"Date must be on or after {validation.date_min}",
        ...
        for section in sec_list:
            for question in section.questions:
                ...
                if question.field_type in [
                    ...
                    try:
                        if v.min_value is not None:
                            kwargs["ge"] = float(v.min_value)
                        if v.max_value is not None:
                            kwargs["le"] = float(v.max_value)
                ...
                if question.field_type in [
                    ...
                    if v.min_selection is not None:
                        kwargs["min_length"] = v.min_selection
                    if v.max_selection is not None:
                        kwargs["max_length"] = v.max_selection

                # Basic validation
                if v.min_length is not None:
                    kwargs["min_length"] = v.min_length
                if v.max_length is not None:
                    kwargs["max_length"] = v.max_length
                if v.regex:
                    kwargs["pattern"] = v.regex
                ...
                if question.field_type in [
                    ...
                    validators.append(
                        BeforeValidator(lambda val, v_obj=v: date_validator(val, v_obj))
                    )
                ...
                if question.field_type in [
                    ...
                    validators.append(
                        AfterValidator(
                            lambda val, v_obj=v: word_count_validator(val, v_obj)
                        )
                    )

                for validator in validators:
                    p_type = Annotated[p_type, validator]
                ...
                if v.requires_confirmation:
                    ...
                    fields[confirm_var] = (
                        p_type,
                        Field(description=f"Confirm {question.label}", default=...),
                    )
