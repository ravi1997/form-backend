...
class LocalKMSProvider(AbstractKMSProvider):
    ...
    def __init__(self, key_ring):
        ...
        if not valid_keys:
            logger.warning(
                "No valid KMS keys found in environment. Generating ephemeral key (data loss risk on restart)."
            )
...
class AWSKMSProvider(AbstractKMSProvider):
    ...
    def batch_decrypt(self, cipher_texts: list[str]) -> list[str]:
        return LocalKMSProvider([settings.FIELD_ENCRYPTION_KEY]).batch_decrypt(
            cipher_texts
        )
