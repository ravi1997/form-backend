...
class ConditionEvaluator:
    ...
    def _evaluate_group(self, condition: Any) -> bool:
        ...
        if logical_op == "AND":
            ...
            return (
                not all(results)
                if len(results) > 1
                else not (results[0] if results else False)
            )
        elif logical_op == "NOR":
    ...
    def _apply_operator(
        self, source_val: Any, operator: str, comparison_val: Any
    ) -> bool:
        ...
        try:
            ...
            if operator == "not_contains":
                return c_str not in s_str
            if operator == "starts_with":
                return s_str.startswith(c_str)
            if operator == "ends_with":
                return s_str.endswith(c_str)
