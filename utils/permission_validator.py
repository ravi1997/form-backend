...
class PermissionValidator:
    ...
    def __new__(cls, *args, **kwargs):
        if not cls._instance:
            cls._instance = super(PermissionValidator, cls).__new__(
                cls, *args, **kwargs
            )
    ...
    def _load_config(self):
        ...
        if not os.path.exists(self.config_path):
            app_logger.warning(
                f"Permissions config not found at {self.config_path}, fallback to empty permissions."
            )
            return
        ...
        try:
            with open(self.config_path, "r") as f:
                ...
                if config:
                    ...
                    app_logger.info(
                        f"Loaded {len(self.roles_config)} roles and {len(self.protected_routes)} protected routes from permissions.yaml"
                    )
        except Exception as e:
            error_logger.error(
                f"Failed to load permissions configuration: {e}", exc_info=True
            )

    def _expand_all_roles(self):
        """Expands permissions for all roles based on the inheritance tree."""
        for role in self.roles_config:
            self.role_expanded_permissions[role] = self._expand_role_permissions(
                role, set()
            )
    ...
    def _expand_role_permissions(self, role: str, visited: Set[str]) -> Set[str]:
        ...
        for parent_role in role_data.get("inherits", []):
            permissions.update(
                self._expand_role_permissions(parent_role, visited.copy())
            )
    ...
    def validate_route_access(
        self, user_roles: List[str], method: str, path: str
    ) -> bool:
        """
